...
class ControllerPaymentPagseguro extends Controller {
    ...
    public function confirm() {
        if ($this->session->data['payment_method']['code'] == 'pagseguro') {
            ...
            $this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('config_order_status_id'),$pagamento, true);
            ...
            switch($strType) {
                ...
                case 'TRANSACTION':
                    ...
                    try {
                        ...
                        $comment = "Código da transação: " . $codigo_transacao . "<br />Tipo de pagamento: " . $payment_code->getTypeFromValue()."<br />Parcelas: ".$parcelas . "\n\n";
                        ...
                        if($pagSeguroShippingType->getValue() != 3){
                            $comment .= "<br />Tipo de frete escolhido no PagSeguro: " . $pagSeguroShippingType->getTypeFromValue() . "<br />Valor do frete: " . $this->currency->format($valor_frete, $order['currency_code'], false, false);
                        }
